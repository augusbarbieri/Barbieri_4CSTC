...
-- 1) Crear un SP que liste todos los productos que tengan un precio de compra mayor al precio promedio
-- y que devuelva la cantidad de productos que cumplan con esa condición.

	/*DELIMITER //
    
CREATE PROCEDURE MayorAlPromedio(OUT cantidad int)
		BEGIN
...
/*call MayorAlPromedio(@cantidad);
select @cantidad;*/

-- 2) Crear un SP que reciba un orderNumber y borre la orden correspondiente a ese orderNumber y todos
-- los ítems de la tabla orderDetails asociados a él. Tiene que devolver 0 si no encontró filas para ese
-- orderNumber, o la cantidad ítems borrados si encontró el orderNumber. */

/*
	DELIMITER //

	CREATE PROCEDURE Borrar(IN orderNum int, OUT cantidad int)
    BEGIN
...
Tenga en cuenta que la línea de productos no podrá ser borrada si tiene productos asociados.
El procedure debe devolver un mensaje que contenga una de las siguientes leyendas:
La línea de productos fue borrada
La línea de productos no pudo borrarse porque contiene productos asociados. */
	/*
	DELIMITER &&

		CREATE PROCEDURE BorrarLinea(IN linea varchar(45), OUT mensaje varchar(70))
		BEGIN
...
/*
	DELIMITER &&
		...
		CREATE PROCEDURE CantidadOrdenes()
		BEGIN
    ...
    -- call CantidadOrdenes();
	
    
-- 5) Al procedimiento anterior agreguele dos parametros de entrada, dateOrder desde hasta, el
-- procedimiento entonces tendrá que listar la cantidad de ordenes por estado entre esas dos fechas.
/*
	DELIMITER &&
    
		CREATE PROCEDURE CantidadOrdenes(IN FechaDesde DATE, IN FechaHasta DATE)
		BEGIN
...
-- 6) Realice un SP que liste para cada empleado con gente subordinada, cuantos empleados tiene a cargo.
...
/*
	DELIMITER &&
		...
		BEGIN
			...
			SELECT sub.employeeNumber, COUNT(jefe.reportsTo)
			from employees as jefe
  			join employees as sub
    			on jefe.reportsTo = sub.employeeNumber
...
-- 7) Realice un SP que liste el numero de orden y su precio total.
/*
	DELIMITER &&
    
		CREATE PROCEDURE NumOrdenPrecio()
		BEGIN
...
-- 8) Crear un SP que liste el numero de cliente y nombre, junto son las ordines asociadas a ese cliente y el
-- total por orden.
/*
	DELIMITER &&
    
		CREATE PROCEDURE ClienteOrdenPrecio()
		BEGIN
        
			SELECT c.customerNumber, c.customerName, o.orderNumber, SUM (odts.quantityOrdered * odts.priceEach)
			from customers as c
			join orders as o
			on c.customerNumber = c.customerNumber
			join orderdetails as odts
			on o.orderNumber = odts.orderNumber
...
-- 9) Realizar un SP que modifique el campo comments de la tabla orders. El procedimiento recibe un
-- orderNumber y el comentario. El procedimiento devuelve 1 si se encontró la orden y se modifico y 0 en
-- caso contrario.
/*
	
    DELIMITER &&
    
		CREATE PROCEDURE ModifComment(IN NumOrden int, IN comentario TEXT, OUT realizado BOOLEAN)
		BEGIN
            
			DECLARE aparece INT DEFAULT 0;            		

			SELECT COUNT(*) INTO aparece
			FROM orders
